...
const socialLinks = [
  {
    ...
    subText: "on LinkedIn",
    icon: Linkedin,
    url: "https://www.linkedin.com/in/rahaf-alwadai-929601250", 
    color: "#0A66C2",
  ...
  },
  {
    ...
    subText: "@itsrav__", 
    icon: Twitter,
    url: "https://x.com/itsrav__?s=11", 
    color: "#ffffff",
    gradient: "from-[#000000] via-[#14171A] to-[#657786]",
  },
  {
    name: "GitHub",
    displayName: "Github",
  ...
  },
  {
    ...
    url: "https://www.tiktok.com/@itsthecsrav?is_from_webapp=1&sender_device=pc",
    color: "black",
...
const SocialLinks = () => {
  ...
  const [twitter, github, tiktok] = otherLinks; // الترتيب الجديد
  ...
  return (
    <div className="w-full bg-gradient-to-br from-white/10 to-white/5 rounded-2xl p-6 py-8 backdrop-blur-xl">
      ...
      <div className="flex flex-col gap-4">
        ...
        {/* Second Row - Twitter & GitHub */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[twitter, github].map((link, index) => (
            <a
              key={link.name}
              href={link.url}
              target="_blank"
        ...
        <div className="grid grid-cols-1 gap-4">
          <a
            ...
            data-aos-delay="400"
          >
            <div className={`absolute inset-0 opacity-0 group-hover:opacity-20 transition-opacity duration-500 bg-gradient-to-r ${tiktok.gradient}`} />
            <div className="relative flex items-center justify-center">
              <div className="absolute inset-0 opacity-20 rounded-lg transition-all duration-500 group-hover:scale-125 group-hover:opacity-30" style={{ backgroundColor: tiktok.color }} />
              <div className="relative p-2 rounded-lg">
                <tiktok.icon className="w-5 h-5 transition-all duration-500 group-hover:scale-110" />
              </div>
            </div>
            <div className="flex flex-col min-w-0">
              <span className="text-sm font-bold text-gray-200 group-hover:text-white transition-colors duration-300">{tiktok.displayName}</span>
              <span className="text-xs text-gray-400 truncate group-hover:text-gray-300 transition-colors duration-300">{tiktok.subText}</span>
            </div>
            <ExternalLink className="w-4 h-4 text-gray-500 group-hover:text-white ml-auto opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:translate-x-0 -translate-x-2" />
          </a>
        </div>
